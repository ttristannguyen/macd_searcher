...
| *(derived)* **confidence** | — | **There is deliberately no `fire_confident` column.** The high-confidence cohort (v2) is a predicate over `direction`, `fire_macd`, `fire_hist` and `fire_reduction_from_peak`, with ATR reaching it through the `asset_snapshots` join — defined once in `signals.is_high_confidence` (Python, for Telegram) and mirrored as `perf._CONFIDENCE_SQL`, which *imports* the same `CONFIDENCE_*` thresholds so the numbers have a single home. Deriving means a threshold change re-labels all history at once instead of needing a re-backfill; the ATR join covers 100% of post-fix signals, so nothing is stored. A test asserts the two implementations agree row-for-row. See [confidence_v2.md](confidence_v2.md) (and [confidence.md](confidence.md) for the retired v1). |
| *(derived)* `fire_macd − fire_hist` | — | **The MACD signal line at fire.** There is deliberately no `fire_macd_signal` column: `indicators.macd` builds `hist = macd − signal`, and both operands above come from the same fire-view bar, so subtraction recovers it exactly — for every signal ever logged, no migration or backfill. Normalized by `asset_snapshots.atr` it drives the signal-line bucket analysis (`perf.macd_signal_buckets`). See [macd_signal_analysis.md](macd_signal_analysis.md). It is bucketed two ways, both derived, neither stored: `÷ atr` (`perf.macd_signal_buckets`, comparable across classes; bucketed as equal-n octiles whose labels carry their measured range and shift with the class filter) and `÷ fire_close × 100` (`perf.macd_signal_pct_buckets`, more legible but partly an asset-class proxy) — see [macd_signal_pct_analysis.md](macd_signal_pct_analysis.md). |
