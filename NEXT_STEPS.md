...
- [x] **by-class** (win/EV by asset class × stage) and **reduction buckets** (win/EV by histogram reduction-from-peak bucket) rendered on the Outcomes tab (`ByClass`, `Thresholds`). *(Stage-3 proximity bucket removed with Stage 3.)*
...
- [~] **Counterfactual loosening** — score `asset_snapshots` (not just fired signals) so "what if thresholds were looser?" is answerable with real returns. *(S1/reduction first cut shipped: `/api/perf/reduction-counterfactual` + the Reduction-counterfactual panel — self-join forward returns from stored closes, extending the reduction buckets below the 0.3 fire threshold. EV/win faithful; drawdown is a close-based proxy. Still open: bar-based true MAE.)*
